...
In particular, when $\Theta = \{\alpha\}$, $X^L_{\alpha, \psi}/Z(X_\alpha)$ has rank one, so it only has one spherical root. It falls into the classification above up to central isogeny, with twisted unipotent. The cuspidal adjoint-type $X^L_{\alpha, \psi} \surj X^{L,ad}_{\alpha, \psi}$ is exactly one of the four above. Hence in this case, the statement above is true.

# Matching of Orbits

## Matching of L and G

 $X^L_{\Theta, \psi}/(N_L, \psi) \into X/(N,\psi)$ since the stabilizer lies in $N_L$, the problem is from $X^{L,ad}_{\Theta, \psi}/(N_L, \psi)$ to $X^L_{\Theta, \psi}/(N_L,\psi)$. We have $X^{L,der}_{\Theta, \psi} \times Z(L)^0 \to X^L_{\Theta, \psi}$ an isogeny.
