...
Assume $S_X = 0$ and $P(X)=B$ and $X$ affine homogeneous first (Call it assumption *). $F$ is a number field. Then $X = H \backslash G$ and $X_1$ is $G_X$ Whittaker. We ignore the rationality problem for now.  
...
$X^L_\Theta$ has the same rank as $X$, and has spherical roots $\Theta$ and $Z(X_\Theta) = \Theta ^\perp \subset X^*(A_X)$.
...
To match this classification, the idea is to reduce to cuspidal rank one adjoint type first, then use some $p$-adic diffeomorphism to go back to $X$ (as CCL or Jacquet did for $SL_2 \times Z_{GL_2} \surj GL_2$ to separate the toric part and tangent part) one first reduces to the adjoint type $X^L_\psi \surj X^L_\psi/Z(L)^0$,  
