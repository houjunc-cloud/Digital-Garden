...
## Matching of L and G
 ...
 $X^L_{\Theta, \psi}/(N_L, \psi) \into X/(N,\psi)$ since the stabilizer lies in $N_L$, the problem is from $X^{L,ad}_{\Theta, \psi}/(N_L, \psi)$ to $X^L_{\Theta, \psi}/(N_L,\psi)$. We have $X^{L,der}_{\Theta, \psi} \times Z(L)^0 \to ^L_{\Theta, \psi}/(N_L,\psi)$
