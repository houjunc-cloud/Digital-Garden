It is (self dual) $\Delta = (SO_{4n}, Sp_{2n}, (2^{2n}), 0)$ with $\Delta_1 = (SO_{2n+1}, 1, \iota_{reg}, 0)$, and $M_{Sh,GL_{2n}} = (GL_{2n}, GL_n, (2^n), 0)$ with the same $M_1 = \Delta_1$. $P$ has Levi $L = GL_{2n}$ and $G = SO_{4n}$. Moreover we have a family on the spectrum side $\mathcal M \to \mathbb A^1$, whose fibers are $\mathcal M_{t} = T^*_{t\lambda_S}(S \backslash G)$, then $\mathcal M_{gn} = M = M_{Sh, SO_{4n}}$ (Whittaker induction) and $\mathcal M_{sp} = T^*(S \backslash G) = Ind_P^G T^*(Sp_{2n} \backslash GL_{2n})$ (parabolic induction). The goal is to descend from $M_{Sh,GL_{2n}}$ to $M_1$. Notice that here we are taking the residue of Eisenstein series at the pole $s = 1/2$, so **.The following explains the Bessel reduction step, i.e. a pre-step $M{/\!\!/}_{\lambda_B}U_B = M_1$. Finally $(M{/\!\!/}_{\lambda_B}U_B){/\!\!/}_{\lambda_{N_H}}N_H \isom M{/\!\!/}_{ \lambda_E} E = Wh_G = Wh_{G_X}$, where the second identity is by the root exchange lemma, and $E \bijection (2n,2n)$ . 
