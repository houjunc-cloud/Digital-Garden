It is (self dual) $\Delta = (SO_{4n}, Sp_{2n}, (2^{2n}), 0)$ with $\Delta_1 = (SO_{2n+1}, 1, \iota_{reg}, 0)$, and $M_{Sh,GL_{2n}}$ with $M_1 = M_{\Delta_1}$. $P$ has Levi $GL_{2n}$ and $G = SO_{4n}$. Moreover we have a family on the spectrum side $\mathcal M \to \mathbb A^1$, whose generic fiber is $\mathcal M_t = T^*_{t\lambda_S}(S \backslash G)$, then $\mathcal M_1 = \mathcal M_{Sh, SO_{4n}}$ (Whittaker induction) and $\mathcal M_0 = T^*(S \backslash G) = Ind_P^G T^*(Sp_{2n} \backslash GL_{2n})$ (parabolic induction).
