...
All the above are of type T except 2.
...
# Matching of Orbits [[Remaining Problems#^orbit-problem]]
...
2. There is one more improvement on 1, we don't really need to find a $G \times H$ space: $M^\vee = (T^*G^\vee \times S_X)//G_X^\vee$, so we already have $M^\vee///_0G^\vee \isom S_X///_0 G_X^\vee = M_1^\vee///_0 G_X^\vee$. Applying Venkatesh's principle we have $M//_{Wh} G = M_1//_{Wh} G_X$. The correct object to consider is $M^\vee = T^*(G_X^\vee \backslash G^\vee \times G_X^\vee) \isom T^*G^\vee$ when $S_X = 0$, which becomes a hyperspherical Hamiltonian space when $(G^\vee, G_X^\vee)$ is a strongly spherical pair.^9cda03
3. (abandoned) universal centralizer + BZSV duality, old way.  ^39b499
...
>[!Proof Program]
>1. $\Phi$ is a symplectic isomorphism that identifies the relevant Lagrangians on both sides (this is wrong, a counterexample is G-J comparison with Hecke. But MWZ is still right; the comparison is by Fourier transform + Poisson +).
...
>4.2 the quantizations are generated by orbital integrals.
> 
>With this, the function space matching is completely organized by the geometric space itself.
...
For split type T case, we have two colors counted in multiplicity, and $\gamma^\vee = \rho(D^+) + \rho(D^-)$. The simplest strict containment appears in $X = GL_2 \backslash GL_3$. The Whittaker dimension adds one on the limit locus, and the Lagrangian fiber becomes Lagrangian surface in this case. However, the two colors have same direction in essential, i.e. after modulo $Z(L)^0$, $\rho(D^+) = \rho(D^-) = \gamma^\vee /2$, and the toric morphism becomes a finite morphism $\mathbb A^1 \to \mathbb A^1: s \mapsto s^2$. There's no problem on the dimension. 
