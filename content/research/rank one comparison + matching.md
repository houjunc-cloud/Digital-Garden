...
2. There is one more improvement on 1, we don't really need to find a $G \times H$ space: $M^\vee = (T^*G^\vee \times S_X)//G_X^\vee$, so we already have $M^\vee///_0G^\vee \isom S_X///_0 G_X^\vee = M_1^\vee///_0 G_X^\vee$. Applying Venkatesh's principle we have $M//_{Wh} G = M_1//_{Wh} G_X$ ^9cda03
...
In the special case when there is a $G \times G_X$ Hamiltonian space $\tilde{M}$ such that $\tilde{M}///_0 G = M$ and $\tilde{M}///_0 G_X = M_1$ (for example $\tilde{M}$ is a minimal orbit), 1 and 2 are automatic.
