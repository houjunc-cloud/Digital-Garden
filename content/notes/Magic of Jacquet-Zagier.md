...
On LHS, its spectral expansion is $P_{H_\epsilon} \circ R(f) \cdot \overline{ B_{s}^H}$. Similarly, after normalization $\tild{B^H_s}(\phi) = W_{\phi}(1) + \sum_{N \mathfrak a >1} W_{\phi}(\mathfrak a)$