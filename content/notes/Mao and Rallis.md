...
GPT guesses, ignoring the hypersphericality condition, $\mathcal O_{min}(G') \bijection T^*(U_\iota \backslash G^\vee, \Psi_\iota) = WI^{G^\vee}_{H^\vee, \iota'^\vee}(T^*H^\vee)$ since Hamiltonian reduction and induction are adjoint to each other (?).
