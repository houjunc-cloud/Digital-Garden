...
We view the orbital integrals of $G$ and $H$ as linear functionals on $\Pi_{U_\theta, \psi}$ and $\Pi_{U_{\iota'},tr}$ both factoring through $Q:=\Pi_{U_\theta, \psi,U_{\iota'},tr}$. So now we are in the setup that we have two families $\{\mathcal O\}, \{\\} \subset Q^\vee$
