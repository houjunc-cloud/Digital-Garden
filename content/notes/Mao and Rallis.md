...
1. Compute $I = q^* \mathcal O$ and $J = q^* \mathcal O_1$ as an integral distribution on $\Pi$ (minimal representation is Weil representation). Because $q$ is surjective, $\mathcal O = \Delta_{\mathcal O} \mathcal O_1 \iff I = \Delta_{\mathcal O} J$. And the problem becomes a calculus problem
2. For the explicit integral form of $I$, the group action on $\Phi \in \Pi$ can be computed explicitly by partial Fourier transform, scaling, translation.
