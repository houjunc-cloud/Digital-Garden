...
-(extended BZSV) According to BEZRUKAVNIKOV-YUN, for any $\psi = \psi_\Theta$, where $\psi_\Theta$ is the generic character of $U_\Theta$ extending trivially to the whole $U$, degenerate Whittaker $(U, \psi_\Theta) \backslash G \bijection [P_\Theta^\vee, P_\Theta^\vee] \backslash G^\vee$ or $T^*_{\psi_\Theta}(U\backslash G) \bijection T^*([P_\Theta^\vee, P_\Theta^\vee] \backslash G^\vee)$ with $G \times A_\Theta$ -action and $G^\vee \times A_\Theta^\vee$ -action respectively, where $A_\Theta = Z(L_\Theta)^0$. After D/N duality on $A_\Theta$-action then $(U, \psi_\Theta) \backslash G = (U_{L,\Theta}) \times ^{P_\Theta} G \bijection pt \times ^{P_\Theta^\vee} \backslash G^\vee = P_\Theta^\vee \backslash G^\vee$
