...
To recover $J_X$,  it suffices to eliminate the extra indices. $(\mathfrak h^\perp)_{H-reg} \subset \mathfrak g_{reg}^* \cap \mathfrak h^\perp$ from the lemma above only . So birationally $R_c = H \backslash \mathfrak h^\perp_{H-reg} \cap \chi_X^{-1}(c)$. Denote by $R_X:= \mathfrak h^\perp_{H-reg} // H$ the regular quotient, we also need to assume $R_X \to \mathfrak c_X$ is birational and surjective. 