...
-(extended BZSV) According to BEZRUKAVNIKOV-YUN, for any $\psi = \psi_\Theta$, where $\psi_\Theta$ is the generic character of $U_\Theta$ extending trivially to the whole $U$, $(U, \psi_\Theta) \backslash G = ((U_\Theta, \psi_\Theta) \backslash L_\Theta) \times^{P_\Theta} G \bijection ((U_\Theta, \psi_\Theta) \backslash L_\Theta) \times^{P_\Theta^\vee} G^\vee$
