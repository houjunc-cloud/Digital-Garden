...
两条线几乎同时起步：谱路线的起点是 [Bruna et al. 2014](https://arxiv.org/abs/1312.6203)，嵌入路线的起点是同年的 DeepWalk。但嵌入路线立刻实用，谱路线要到 GCN（2017）才落地。**它们最后被证明是同一件事的两个面**，见第 5 节。
...
$$x^\top L x=\sum_{(u,v)\in E}(x_u-x_v)^2=\tfrac12\sum_{u,v}A_{uv}(x_u-x_v)^2 .$$
...
- **加自环 $A+I$** 不是可有可无的：$D^{-1/2}AD^{-1/2}$ 的谱含 $-1$ **当且仅当**图有二部分支；此时 $I+D^{-1/2}AD^{-1/2}$ 在对应的特征方向上退化为 $0$，那一部分信息被直接抹掉。给每个节点加自环后图不再二部，于是 $\lambda_{\min}(\hat A)>-1$。附带的好处是谱从 $[0,2]$ 收缩到 $(-1,1]$，深层堆叠不再逐层放大——这正是 Kipf–Welling 提出这个技巧的实验动机。
...
**定理（过平滑，[Oono–Suzuki 2020](https://arxiv.org/abs/1905.10947) 的简化陈述）.** 设 $\mathcal M$ 是 $\hat A$ 的最大特征值对应的不变子空间（对连通非二部图，由 $\tilde D^{1/2}\mathbf 1$ 张成），$\lambda:=\max\{|\lambda_i|:\lambda_i\ne1\}$ 是 $\hat A$ 的**非单位特征值中模最大者**（注意可能由最负的那个达到，未必是次大的），并记 $s:=\sup_l\|W^{(l)}\|_2$。则
...
（证明能穿过非线性，是因为 $\mathrm{ReLU}$ **不增加**到 $\mathcal M$ 的距离：$d_{\mathcal M}(\sigma(X))\le d_{\mathcal M}(X)$，用到张成 $\mathcal M$ 的特征向量可取非负。$\mathrm{ReLU}$ 是到非负卦限的投影，不是到 $\mathcal M$ 的投影——$\mathcal M$ 是线性子空间而 $\mathrm{ReLU}$ 非线性。）
...
$$h_{\mathcal N(v)}^{k}=\mathrm{AGGREGATE}_k\big(\{h_u^{k-1}:u\in\mathcal N(v)\}\big),\qquad h_v^{k}=\sigma\!\Big(W^k\cdot\mathrm{CONCAT}\big(h_v^{k-1},\,h_{\mathcal N(v)}^{k}\big)\Big),$$
...
$$e_{ij}=\mathrm{LeakyReLU}\big(\vec a^\top[Wh_i\,\|\,Wh_j]\big),\qquad \alpha_{ij}=\frac{\exp(e_{ij})}{\sum_{k\in\mathcal N_i}\exp(e_{ik})},$$
...
于是 **GIN**：$h_v^{(l+1)}=\mathrm{MLP}\big((1+\epsilon)h_v^{(l)}+\sum_{u\in\mathcal N(v)}h_u^{(l)}\big)$，是**本节列出的架构里**唯一达到上界的（一般的充分条件就是上面定理里的两个单射性，GIN 只是最简单的实现）。
...
原文用 **Hierarchical Softmax**（沿 $V$ 上的一棵 Huffman 树把 softmax 分解为 $O(\log|V|)$ 次二分类）处理 $|V|$ 项的归一化。**负采样**是另一条路，但它不是对 softmax 的分解而是换成一个二分类代理目标，代价 $O(k)$；DeepWalk 本身没用它，它从 node2vec 才进入这条线。
...
$$\log\left(\frac{\mathrm{vol}(G)}{T}\sum_{r=1}^{T}\big(D^{-1}A\big)^{r}D^{-1}\right)-\log b,$$
