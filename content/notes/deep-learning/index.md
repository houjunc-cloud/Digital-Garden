...
- **图与几何**（想做分子、材料、关系数据）：10 → 24 → 12
- **生成模型优先**（想理解 diffusion / flow）：03 → 04 → 14 → 15 → 17

> [!tip] 每篇开头都有一个「卡住了从哪儿看起」的框
> 里面是 2–5 个外部链接：课程讲义、博客、视频、原论文，各自标了适合什么情况。正文里重要概念旁边也直接挂了链接。总表见 **[[00 外部资源地图]]**。
>
> 用法上的一个建议：**先找视频或图解建立直觉，再回来看这里的严格版本。**倒过来做通常更慢。
>
> 视频课单列在 **[[00b 视频网课地图]]**，每一章都对应了一到三条「先看这个」。

## 模块

### 资源

- [[00 外部资源地图]] — 所有外部课程、讲义、博客的分主题总表
- [[00b 视频网课地图]] — **逐章配一门视频课**（中英文对照），卡住的时候先看这里
- [[00c 16 周执行表]] — **2026-09-08 → 12-25 的逐周动手计划**，这套讲义在那 16 周里当查阅手册用
- [[A 概率与统计不等式手册]] — 附录：全部用到的概率统计不等式的集中证明与依赖图
...
- [[24 图上的学习]] — 图拉普拉斯与谱卷积、GCN 的推导与过平滑定理、消息传递（GraphSAGE/GAT/GIN）、图嵌入（DeepWalk/LINE/node2vec/SDNE）与它们统一为矩阵分解
