...
The theta pairing of $X_{grp}$ and $X_{Wh}$ is equivalent to one theta series on $X_{RS} = V \times ^{\Delta GL_2} (GL_2 \times GL_2)$ with an $s$-twisting parameter (somewhere in the ICM talk of Sakellaridis). The latter has two $G$ orbits $\{0\}$ and $V \backslash \{0\}$. On $\{0\}$ the model becomes $\{0\}\times ^{\Delta GL_2} (GL_2 \times GL_2) \isom X_{grp}$  